<script lang="ts">
	...
	import { ALL_TREATMENT_CATEGORIES, TREATMENT_CATEGORY_LABELS } from '$lib/shared/enums';
	import { getFormErrorMessage } from '$lib/utils';
	import type { SupplierTreatment } from '$lib/server/db/schema';
	import type { updateSupplierTreatmentForm } from '$lib/remote/suppliers.remote';

	interface Props {
		treatment: SupplierTreatment;
		form: ReturnType<typeof updateSupplierTreatmentForm.for>;
		taxable: boolean;
	...
	let { treatment, form, taxable, onToggleTaxable, onCancel, onResult }: Props = $props();
</script>
...
>
	...
	<div>
		<label for="edit-name-{treatment.id}" class="mb-1 block text-[11px] font-medium text-slate-500"
			>Nombre</label
		>
		<input
