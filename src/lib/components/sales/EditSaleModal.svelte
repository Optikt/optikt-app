<script lang="ts">
	...
	import { Package, Eye, Sparkles, FlaskConical, Plus, X, Pen, Pencil, Save } from '@lucide/svelte';
	...
	/** Bridge between EditableItem (number/os*) and PrescriptionInput (string/oi*) */
	...
	const editFieldInputClass =
		'w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:border-brand-blue focus:ring-1 focus:ring-brand-blue focus:outline-none';
	...
	function populatePrescriptionValues() {
		const lens = editLensTmp.lensCatalogItemId
			? lensItems.find((l) => l.id === editLensTmp.lensCatalogItemId) ?? null
			: null;
		prescriptionValues = {
			odSphere: editLensTmp.odSphere?.toString() ?? '',
			odCylinder: editLensTmp.odCylinder?.toString() ?? '',
			odAxis: editLensTmp.odAxis?.toString() ?? '',
			odAddition: editLensTmp.odAddition?.toString() ?? '',
			oiSphere: editLensTmp.osSphere?.toString() ?? '',
			oiCylinder: editLensTmp.osCylinder?.toString() ?? '',
			oiAxis: editLensTmp.osAxis?.toString() ?? '',
			oiAddition: editLensTmp.osAddition?.toString() ?? '',
			lensType: lens?.type ?? LensType.MONOFOCAL,
			doctorName: ''
		};
	}

	function readPrescriptionValues() {
		editLensTmp.odSphere = parseFloat(prescriptionValues.odSphere) || undefined;
		editLensTmp.odCylinder = parseFloat(prescriptionValues.odCylinder) || undefined;
		editLensTmp.odAxis = prescriptionValues.odAxis ? parseInt(prescriptionValues.odAxis) : undefined;
		editLensTmp.odAddition = parseFloat(prescriptionValues.odAddition) || undefined;
		editLensTmp.osSphere = parseFloat(prescriptionValues.oiSphere) || undefined;
		editLensTmp.osCylinder = parseFloat(prescriptionValues.oiCylinder) || undefined;
		editLensTmp.osAxis = prescriptionValues.oiAxis ? parseInt(prescriptionValues.oiAxis) : undefined;
		editLensTmp.osAddition = parseFloat(prescriptionValues.oiAddition) || undefined;
	}

	function startLensEdit(item: EditableItem) {
		editingLensId = item.id ?? null;
		editLensTmp = { ...item };
		populatePrescriptionValues();
		// Load existing treatments for this lens
	...
	function handleLensSelect(id: string, unitPrice: number) {
		...
		// Clear treatments when lens changes (supplier may differ)
		if (id && !editingLensId) {
			editLensTreatments = [];
		}
	}
	...
	function addTreatmentToEdit() {
		// Find first available treatment not already selected
	...
	const fieldInputClass =
		'w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-brand-blue focus:ring-1 focus:ring-brand-blue focus:outline-none';
</script>

<Modal bind:open size="xl" title="Editar Venta" permanent={saving || !!editingLensId}>
	<div class="space-y-6">
		<!-- Info header -->
		<div class="rounded-xl bg-info-container/40 px-4 py-3 text-sm text-on-info-container">
			<p class="font-semibold">
				Editando venta <strong>#{String(sale.orderNumber).padStart(4, '0')}</strong>
			</p>
		...
		<div class="grid gap-4 md:grid-cols-2">
			<div>
				<label for="edit-sale-date" class={fieldLabelClass}>Fecha de venta</label>
				<input id="edit-sale-date" type="date" bind:value={saleDate} class={`${fieldInputClass} mt-1.5`} />
			</div>
			<div>
				<label for="edit-sale-notes" class={fieldLabelClass}>Observaciones</label>
				<input id="edit-sale-notes" type="text" bind:value={notes} placeholder="Notas opcionales..." class={`${fieldInputClass} mt-1.5`} />
			</div>
		</div>
		...
		<div class="grid gap-4 md:grid-cols-3">
			<div>
				<label for="edit-discount-type" class={fieldLabelClass}>Tipo de descuento</label>
				<select id="edit-discount-type" bind:value={discountType} class={`${fieldInputClass} mt-1.5`}>
					<option value={DiscountType.FIXED}>Monto fijo ($)</option>
					<option value={DiscountType.PERCENTAGE}>Porcentaje (%)</option>
				</select>
			</div>
			<div>
				<label for="edit-discount" class={fieldLabelClass}>Descuento {discountType === DiscountType.PERCENTAGE ? '(%)' : '($)'}</label>
				<input id="edit-discount" type="number" bind:value={discount} min="0" step="0.01" class={`${fieldInputClass} mt-1.5`} />
			</div>
			<div class="flex items-end pb-3">
				<p class="text-sm text-on-surface-variant">Cliente: {sale.customer?.firstName} {sale.customer?.lastName}</p>
			</div>
		</div>
		...
		<div>
			<div class="flex items-center justify-between">
				<p class={fieldLabelClass}>Artículos ({activeItems.length})</p>
				<div class="flex gap-2">
					<button type="button" onclick={() => { closeAllAddFormsExcept('lens'); startLensAdd(); }}
						class="inline-flex items-center gap-1.5 rounded-xl bg-info-container px-3.5 py-2 text-xs font-bold text-on-info-container transition-colors hover:bg-info-container/80">
						<Plus class="h-3.5 w-3.5" /> Cristal
					</button>
					<button type="button" onclick={() => { closeAllAddFormsExcept('product'); showAddProduct = !showAddProduct; }}
						class="inline-flex items-center gap-1.5 rounded-xl bg-brand-navy px-3.5 py-2 text-xs font-bold text-white transition-colors hover:bg-brand-navy-dark">
						<Plus class="h-3.5 w-3.5" /> Producto
					</button>
					<button type="button" onclick={() => { closeAllAddFormsExcept('free'); showAddFreeItem = !showAddFreeItem; }}
						class="inline-flex items-center gap-1.5 rounded-xl bg-amber-600 px-3.5 py-2 text-xs font-bold text-white transition-colors hover:bg-amber-700">
						<Plus class="h-3.5 w-3.5" /> Ítem libre
					</button>
				</div>
			</div>
			...
			<!-- Add product form -->
			{#if showAddProduct}
				<div transition:slide={{ duration: 180 }} class="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
					<p class="mb-3 text-xs font-semibold text-brand-navy">Agregar producto</p>
					<div class="space-y-3">
						...
						<div class="grid grid-cols-3 gap-3">
							<div>
								...
								<input id="add-prod-qty" type="number" bind:value={addProductQty} min="1" class={fieldInputClass} />
							</div>
							<div>
								<label for="add-prod-price" class="mb-1 block text-[10px] font-semibold tracking-wider text-slate-500 uppercase">Precio</label>
								<input id="add-prod-price" type="number" bind:value={addProductPrice} min="0" step="0.01" class={fieldInputClass} />
							</div>
							<div>
								<label for="add-prod-discount" class="mb-1 block text-[10px] font-semibold tracking-wider text-slate-500 uppercase">Desc.</label>
								<input id="add-prod-discount" type="number" bind:value={addProductDiscount} min="0" step="0.01" class={fieldInputClass} />
							</div>
						</div>
						<div class="flex items-center gap-3">
							<select bind:value={addProductDiscountType} class={`${fieldInputClass} w-40`}>
								<option value={DiscountType.FIXED}>Fijo ($)</option>
								<option value={DiscountType.PERCENTAGE}>%</option>
							</select>
							<input type="text" bind:value={addProductNotes} placeholder="Notas (opcional)" class={`${fieldInputClass} flex-1`} />
						</div>
						<div class="flex justify-end gap-2">
							<button type="button" onclick={resetAddProductForm} class="rounded-xl px-4 py-2 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-100">Cancelar</button>
							<button type="button" onclick={addNewProduct} disabled={!addProductId} class="rounded-xl bg-brand-navy px-4 py-2 text-xs font-bold text-white transition-colors hover:bg-brand-navy-dark disabled:opacity-50">Agregar</button>
						</div>
					</div>
				</div>
			{/if}
			...
			<!-- Add free item form -->
			{#if showAddFreeItem}
				<div transition:slide={{ duration: 180 }} class="mt-3 rounded-xl border border-amber-200 bg-amber-50/50 p-4">
					<p class="mb-3 text-xs font-semibold text-amber-800">Agregar ítem libre</p>
					<div class="space-y-3">
						<div class="grid grid-cols-2 gap-3">
							<div>
								...
								<select id="add-free-cat" bind:value={addFreeCategory} class={fieldInputClass}>
							...
							<div>
								...
								<input id="add-free-price" type="number" bind:value={addFreePrice} min="0" step="0.01" class={fieldInputClass} />
							</div>
						</div>
						<div>
							...
							<input id="add-free-desc" type="text" bind:value={addFreeDescription} placeholder="Ej: Funda antivuelco" class={fieldInputClass} />
						</div>
						<div class="grid grid-cols-2 gap-3">
							<div>
								...
								<input id="add-free-discount" type="number" bind:value={addFreeDiscount} min="0" step="0.01" class={fieldInputClass} />
							</div>
							<div>
								...
								<select id="add-free-discount-type" bind:value={addFreeDiscountType} class={fieldInputClass}>
									<option value={DiscountType.FIXED}>Fijo ($)</option>
									<option value={DiscountType.PERCENTAGE}>%</option>
								</select>
							</div>
						...
						<div class="flex justify-end gap-2">
							<button type="button" onclick={resetAddFreeItemForm} class="rounded-xl px-4 py-2 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-100">Cancelar</button>
							<button type="button" onclick={addNewFreeItem} disabled={!addFreeDescription.trim() || addFreePrice <= 0} class="rounded-xl bg-amber-600 px-4 py-2 text-xs font-bold text-white transition-colors hover:bg-amber-700 disabled:opacity-50">Agregar</button>
						</div>
					</div>
				</div>
			{/if}
			...
			{#if showAddLens || editingLensId}
				<div transition:slide={{ duration: 180 }} class="mt-3 rounded-xl border border-sky-200 bg-sky-50/60 p-4">
					<p class="mb-3 text-xs font-semibold text-sky-800">
					...
					<div class="space-y-3">
						...
						<div class="grid grid-cols-4 gap-3">
							<div>
								...
								<input type="number" bind:value={editLensTmp.quantity} min="1" class={editFieldInputClass} />
							</div>
							<div>
								<label class="mb-1 block text-[10px] font-semibold tracking-wider text-slate-500 uppercase">Precio</label>
								<input type="number" bind:value={editLensTmp.unitPrice} min="0" step="0.01" class={editFieldInputClass} />
							</div>
							<div>
								<label class="mb-1 block text-[10px] font-semibold tracking-wider text-slate-500 uppercase">Desc.</label>
								<input type="number" bind:value={editLensTmp.discount} min="0" step="0.01" class={editFieldInputClass} />
							</div>
							<div>
								<label class="mb-1 block text-[10px] font-semibold tracking-wider text-slate-500 uppercase">Tipo desc.</label>
								<select bind:value={editLensTmp.discountType} class={editFieldInputClass}>
									<option value={DiscountType.FIXED}>$</option>
									<option value={DiscountType.PERCENTAGE}>%</option>
								</select>
							</div>
						</div>
						<!-- Prescription (reuses PrescriptionInput) -->
						<PrescriptionInput values={prescriptionValues} compact {showAddition} />
						<!-- Treatments -->
						<div>
							...
							{#if editLensTreatments.length === 0}
								<p class="text-xs text-outline italic">Sin tratamientos seleccionados</p>
							{:else}
								<div class="space-y-1.5">
									{#each editLensTreatments as t, idx (t.supplierTreatmentId + idx)}
										<div class="flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-xs shadow-sm">
											<FlaskConical class="h-3.5 w-3.5 shrink-0 text-purple-600" />
											<span class="flex-1 font-medium text-slate-800">{t.name}</span>
											<span class="font-mono text-slate-600">{formatPrice(t.price)}</span>
											<button type="button" onclick={() => removeTreatmentFromEdit(idx)}
												class="flex h-5 w-5 items-center justify-center rounded text-slate-400 hover:bg-red-50 hover:text-red-600"
						...
						<div class="flex justify-end gap-2 pt-2">
							<button type="button" onclick={cancelLensEdit} class="rounded-xl px-4 py-2 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-100">Cancelar</button>
							<button type="button" onclick={saveLensEdit} disabled={!editLensTmp.lensCatalogItemId || editLensTmp.unitPrice <= 0}
								class="inline-flex items-center gap-1.5 rounded-xl bg-sky-700 px-4 py-2 text-xs font-bold text-white transition-colors hover:bg-sky-800 disabled:opacity-50">
			...
			<div class="mt-3 space-y-2">
				{#each mainItems as item, i (item.id || i)}
					...
					<div
						class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 transition-colors hover:border-slate-300"
						class:opacity-50={editingLensId === item.id}
					>
						<div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg
							{item.itemType === SaleItemType.LENS_PAIR ? 'bg-info-container text-on-info-container' :
							 item.itemType === SaleItemType.FREE_ITEM ? 'bg-amber-100 text-amber-700' :
							 item.itemType === SaleItemType.TREATMENT ? 'bg-purple-100 text-purple-700' :
						...
						<div class="min-w-0 flex-1">
							<div class="flex items-center gap-2">
								...
								{#if !item.id}
									<span class="shrink-0 rounded-full bg-green-100 px-2 py-0.5 text-[9px] font-bold tracking-wide text-green-700 uppercase">Nuevo</span>
								{/if}
							</div>
							{#if itemDetail(item)}
								<p class="mt-0.5 truncate text-xs text-outline">{itemDetail(item)}</p>
							{/if}
						</div>
						<div class="text-right">
							<p class="font-mono text-sm font-semibold text-brand-navy">{formatPrice(item.unitPrice)}</p>
							<p class="text-xs text-outline">x{item.quantity}</p>
						</div>
						{#if item.itemType === SaleItemType.LENS_PAIR}
							<button type="button" onclick={() => startLensEdit(item)} disabled={!!editingLensId}
								class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-sky-50 hover:text-sky-600 disabled:opacity-30"
								title="Editar cristal">
								<Pencil class="h-3.5 w-3.5" />
							</button>
						{/if}
						<button type="button" onclick={() => removeItem(item)} disabled={!!editingLensId}
							class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600 disabled:opacity-30"
							title="Eliminar">
							<X class="h-4 w-4" />
						</button>
					</div>

					<!-- Show treatments under lens -->
					{#if item.itemType === SaleItemType.LENS_PAIR}
						...
						{#each childTreatments as treatment (treatment.id)}
							<div class="ml-8 flex items-center gap-3 rounded-lg border border-dashed border-purple-200 bg-purple-50/40 px-4 py-2">
								<FlaskConical class="h-3.5 w-3.5 shrink-0 text-purple-500" />
								<span class="flex-1 text-xs font-medium text-slate-700">{treatment.snapshotName ?? 'Tratamiento'}</span>
								<span class="font-mono text-xs text-slate-500">{formatPrice(treatment.unitPrice)}</span>
							</div>
						{/each}
					{/if}
				{/each}
				...
				{#if mainItems.length === 0}
					<div class="rounded-xl border border-dashed border-slate-300 py-8 text-center text-sm text-outline">
		...
		<!-- ── Reason (mandatory) ───────────────────────────────────────── -->
		<div>
			<label for="edit-reason" class={fieldLabelClass}>Motivo de la modificación <span class="text-red-500">*</span></label>
			<textarea id="edit-reason" bind:value={reason} oninput={() => (reasonError = '')}
				...
				class={`${fieldInputClass} mt-1.5 resize-none`}></textarea>
	...
	<div class="mt-6 flex items-center justify-between gap-3 border-t border-slate-200 pt-4">
		<div class="text-xs text-outline">
			{#if removedCount > 0}
				<span class="text-red-600">{removedCount} artículo(s) eliminado(s)</span>
			{/if}
		</div>
		<div class="flex gap-2">
			<button type="button" onclick={handleClose} disabled={saving}
				class="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 transition-colors hover:bg-slate-100 disabled:opacity-50">
				Cancelar
			</button>
			<button type="button" onclick={handleSubmit} disabled={saving || !hasChanges}
				class="inline-flex items-center gap-2 rounded-xl bg-brand-navy px-5 py-2.5 text-sm font-bold text-white transition-colors hover:bg-brand-navy-dark disabled:opacity-50">
				{#if saving}
