...
<div class="space-y-6">
	<!-- Row 1: Customer + Quote Info side by side -->
	<div class="grid gap-5 lg:grid-cols-5">
		...
		>
			...
			{:else}
				<p class="text-lg text-slate-400 italic">Sin cliente asignado</p>
				<p class="mt-1 text-sm text-slate-400">Se puede asignar luego antes de convertir a venta</p>
			{/if}
		</div>

		<!-- Quote Details — right column (2/5 width) -->
			...
			>
				{#if nextQuoteNumber}
					<div class="mb-4 flex items-center gap-2">
						...
						<p class="text-xs font-bold tracking-widest text-blue-100 uppercase">Nº Presupuesto</p>
					</div>
					<p class="mb-4 font-mono text-3xl font-bold text-white">P-{nextQuoteNumber}</p>
				{/if}
					...
					<div class="flex flex-1 items-center gap-2">
						...
						<div>
							...
							<p class="font-mono text-sm font-semibold text-white">
								{dateToISODateString(quoteDate)}
							</p>
						</div>
					</div>
					<div class="h-8 w-px bg-white/20"></div>
					<div class="flex flex-1 items-center gap-2">
						<ClipboardList class="h-4 w-4 text-blue-200" />
						<div>
				...
				{#if validUntil}
					<div class="mt-3 flex items-center gap-2 border-t border-white/20 pt-3">
						<Calendar class="h-4 w-4 text-blue-200" />
						<div>
							<p class="text-xs text-blue-200">Válido hasta</p>
							<p class="font-mono text-sm font-semibold text-white">{validUntil}</p>
						</div>
					</div>
				{/if}
			</div>
...
<div class="mt-6 flex justify-between">
	...
	<div class="flex gap-3">
		<Button color="light" size="lg" href={resolve('/quotes')}>Cancelar</Button>
