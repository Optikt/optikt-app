		...
		<table class="min-w-full border-separate [border-spacing:0_0.65rem] text-left">
			<thead>
				<tr>
					...
					<th class="px-3 pb-2 text-right text-[0.65rem] font-bold tracking-[0.18em] text-outline uppercase">
						Existencia
					</th>
					<th class="px-3 pb-2 text-center text-[0.65rem] font-bold tracking-[0.18em] text-outline uppercase">
						Accion
					</th>
				</tr>
			</thead>
			<tbody>
				{#each activeLots as lot, index (lot.id)}
					...
					<tr class="transition-transform duration-150 ease-out hover:-translate-y-px">
						<td class={`rounded-l-xl px-4 py-4 ${selected ? 'bg-surface-container text-brand-navy' : 'bg-surface-container-low text-brand-navy'}`}>
						...
						<td class={`px-4 py-4 text-sm ${selected ? 'bg-surface-container text-brand-navy' : 'bg-surface-container-low text-on-surface-variant'}`}>
							{formatDate(lot.createdAt, { day: '2-digit', month: 'short', year: 'numeric' })}
						</td>
						<td class={`px-4 py-4 font-mono text-sm ${selected ? 'bg-surface-container text-brand-navy' : 'bg-surface-container-low text-on-surface-variant'}`}>
							{formatPrice(lot.unitPurchasePrice)}
						</td>
						<td class={`px-4 py-4 text-right font-mono text-sm font-semibold ${selected ? 'bg-surface-container text-brand-navy' : 'bg-surface-container-low text-brand-navy'}`}>
							{lot.quantityAvailable}
						</td>
						<td class={`rounded-r-xl px-4 py-4 text-center ${selected ? 'bg-surface-container' : 'bg-surface-container-low'}`}>
							<input
