...
## 5. Checklist branch `chore/deps-2026-08`

- [x] Branch creado, DT17 pinneado, plan documentado
- [ ] Tanda 1 — tooling
- [ ] Tanda 2 — styling
- [ ] Tanda 3 — runtime UI
- [ ] Tanda 4 — zod + svelte-check
- [ ] Tanda 5 — svelte/kit/vite core (juntos)
- [ ] Tanda 6 — vitest/playwright
- [ ] Tanda 7 — @oslojs/crypto → node:crypto
- [ ] Push `chore/deps-2026-08` → PR → merge a `main`

Cada tanda = `chore(deps): tanda N — <descripción>` para `git revert` limpio.

## 6. Majors bloqueados — Para rama aparte

- **`typescript 7.0.2`**: pide TS 7 mientras Kit 3 exige TS 6, `svelte-check` y `typescript-eslint` aún no certificados. Esperar Kit 3 stable.
- **`@lucide/svelte 1.34.0`**: `0.x → 1.x` reescribe API `Icon` (150+ imports). Requiere codemod + QA visual. Branch `chore/lucide-1` dedicado.
- **Actions `checkout 7` / `setup-node 7` / `docker/*`**: cambios en `.github/workflows/tests.yml`, probar CI en branch.
- **`puppeteer-core 25.8.0`**: solo con `apt install libnspr4 libnss3` o `CHROMIUM_PATH` validado (ver incidente 2026-08-27).
